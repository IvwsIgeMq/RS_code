...
void dealbuffer (const Context *context, UInt8_t **c, UInt8_t* buf,
	...
	while (pos < buflen) {
		...
		sprintf (name, FORMAT, context->fname, bnum);
		fp = fopen(name, "ab+");
...
void clearBlock (const Context *context, int num) {
	...
	char name[NAME_LEN];
	for (i = 0; i < num; ++i) {
		sprintf (name, FORMAT, context->fname, i);
		remove (name);
	}
}
...
void encodeFile (const Context *context, int n, int m) {
	...
	// 将编码得到的冗余块写入磁盘
	for (i = 0; i < m; ++i) {
		sprintf (name, FORMAT, context->fname, n+i);
		pm = fopen(name, "wb");
...
void buildBlock(matrix_t *A, UInt8_t **era, char *fname, int *ic,
	...
	for (i = 0; i < lenb; ++i) {
		...
		sprintf (name, FORMAT, fname, ib[i]);
		for (j = 0; j < lenc; ++j) {
			coef = A->m_data[i][j];
...
void buildBackups(const Context *context, UInt8_t **era, int *ib,
	...
	for (i = 0; i < m; ++i) {
		...
		for (j = 0; j < lenb; ++j) {
			...
			sprintf (name, FORMAT, context->fname, ib[j]);
			fb = fopen (name, "rb");
			pos = 0;
...
int main (void) {
	...
	// 编码
	encodeFile (&context, 4, 4);
	printf ("input the delete file's index : ");
	fflush(stdin);
	scanf ("%d", &i);
	scanf ("%d", &j);
	sprintf (name1, FORMAT, context.fname, i);
	sprintf (name2, FORMAT, context.fname, j);
